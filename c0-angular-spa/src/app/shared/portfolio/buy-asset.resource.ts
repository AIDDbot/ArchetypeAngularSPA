...
import { DEFAULT_TRANSACTION, Transaction } from "./transaction.type";

@Injectable()
export class BuyAssetResource implements Resource<Transaction> {
  ...
  public buyAsset(portfolioId: string, transaction: CreateTransactionDto): void {
    this.status.set("loading");
    this.error.set(undefined);
    this.http.post<Transaction>(`${this.url}/${portfolioId}/transactions`, transaction).subscribe({
      next: () => {
